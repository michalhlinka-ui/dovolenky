...
// --- Admin login fallback (dočasné pre bootstrapping) ---
const ADMIN_FALLBACK_CODE = "9999"; // zmeň si podľa potreby; povolený ako záložný kód
...
async function loadConfig(){
  try {
    ...
    if(snap.exists()){
      state.data.adminCode = String(snap.data().adminCode || '').trim();
      state.data.lastRolloverYear = snap.data().lastRolloverYear ?? null;
    } else {
      // ešte neexistuje config dokument
      state.data.adminCode = '';
      state.data.lastRolloverYear = null;
    }
  } catch (e) {
    console.warn('loadConfig() error:', e?.message || e);
    // necháme prázdne; login handler to ošetrí
    state.data.adminCode = state.data.adminCode || '';
  }
}
...
// ---- Email notify helper ----
const ADMIN_EMAILS = [
  "hlinka.michal@gmail.com" // ← môžeš doplniť ďalších adminov
];

async function sendNotify({ toList = ADMIN_EMAILS, subject, text, html }) {
  // pošli samostatne na každého príjemcu
...
function setupAuth(){
  ...
  el('#loginAdmin').onclick = async () => {
    ...
    // 1) Bootstrap: ak config chýba, ponúkni nastavenie
    if (!state.data.adminCode) {
      if (!code) { alert('Zadaj admin kód.'); return; }
      if (confirm('Nenašiel sa admin config. Nastaviť zadaný kód ako admin kód?')) {
        await setDoc(doc(db,"config","admin"), { adminCode: code, lastRolloverYear: null });
        state.data.adminCode = code;
      }
    }

    // 2) Over proti uloženému kódu alebo dočasnému fallbacku
    if (code === state.data.adminCode || code === ADMIN_FALLBACK_CODE) {
      ...
      c.addEventListener('click', () => {
        ...
        showHourPicker({
          ...
          onSave: async (hrs)=>{
            ...
            // >>> notifikácia pre admina po zápise
            try {
              const u = getUserById(state.currentUserId);
              const action = (hrs === 0)
                ? 'zrušil žiadosť'
                : (myIdx === -1 ? 'vytvoril žiadosť' : 'upravil žiadosť');
              const hoursTxt = (hrs === 0) ? '' : ` • ${hrs} h`;
              await sendNotify({
                subject: `Dovolenky: ${u?.name || 'Zamestnanec'} ${action} (${dateStr})`,
                text: `${u?.name || 'Zamestnanec'} ${action} na ${dateStr}${hoursTxt}.`
              });
            } catch (e) {
              console.warn('notify fail (employee onSave)', e);
            }
          }
        });
      });
...
async function cycleAdminStatus(dateStr){
  ...
  // ... (ďalej logika pre dovolenky – výber hodín, pending/approved atď.)
  ...
  if(!item){
    ...
    showHourPicker({
      ...
      onSave: async (hrs)=>{
        ...
        try {
          await sendNotify({
            subject: `Dovolenky: ${user.name} – NOVÁ žiadosť (${dateStr})`,
            text: `Admin vytvoril žiadosť pre ${user.name} na ${dateStr} • ${hrs} h. Stav: pending.`
          });
        } catch (e) { console.warn('notify fail (admin create)', e); }
      }
    });
  }else{
    ...
    showHourPicker({
      ...
      onSave: async (hrs)=>{
        if (hrs === 0) {
          ...
          try {
            await sendNotify({
              subject: `Dovolenky: ${user.name} – ŽIADOSŤ ZMAZANÁ (${dateStr})`,
              text: `Admin zmazal žiadosť pre ${user.name} z ${dateStr}.`
            });
          } catch(e){ console.warn('notify fail (admin delete)', e); }
        ...
        try {
          await sendNotify({
            subject: `Dovolenky: ${user.name} – ${item.status.toUpperCase()} (${dateStr})`,
            text: `Admin nastavil ${user.name} dňa ${dateStr} na ${item.status} • ${hrs} h.`
          });
        } catch (e) { console.warn('notify fail (admin toggle)', e); }
      }
    });
  }
}
